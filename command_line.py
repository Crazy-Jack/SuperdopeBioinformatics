...
parser.add_argument('inputFiles1', type=str,
                           help='The first input files')

# Create argument for second input file
parser.add_argument('inputFiles2', type=str,
                           help='The second input files')
...
parser.add_argument('outputFiles2', type=str, help = 'Name of second output file')
# Plan for left adpater and right adpater, waiting for coding
# parser.add_argument('-l', type=str,
#                     help='Left adapater of the file')
# parser.add_argument('-r', type=str,
#                     help='Right adapater of the file')
...
combine_args_skewer = '../skewer/skewer' + ' ' + \
    args.inputFiles1 + ' ' + args.inputFiles2
...
p = subprocess.Popen(combine_args_skewer, preexec_fn=lambda: signal.signal(
    signal.SIGPIPE, signal.SIG_DFL), shell=True).wait()
...
combine_args_inputlist1 = 'echo \"' + args.inputFiles1[:-3] + \
    '-trimmed-pair1.fastq\" > input_list.txt'

combine_args_inputlist2 = ''
combine_args_inputlist2 = 'echo \"' + args.inputFiles1[:-3] + \
...
combine_args_fastuniq = '../FastUniq/source/fastuniq -i input_list.txt -t q -o ' + args.outputFiles1 + ' -p ' + args.outputFiles2 + ' -c 1'

p = subprocess.Popen(combine_args_fastuniq, preexec_fn=lambda: signal.signal(
    signal.SIGPIPE, signal.SIG_DFL), shell=True).wait()
