...
def produce_anno(gtf_file,id_file):
    print("Calling gtf2PRAS")
    print(gtf_file)
    cmd = "/home/snigdhaa/script/gtf2PRAS.sh {0} {1}".format(gtf_file,id_file)
...
def assign_peaks_to_the_tran_id_mergedpeak(annotation_file,binding_file,assignment_file):
    ...
    for line in ifile:
        ...
        hit_position=str(old_div((int(hit_position_s)+int(hit_position_e)),2))#center of peak
        ...
        tran_exon_lens=list(map(int,transcript_to_use[3].split(',')))
        tran_exon_starts=list(map(int,transcript_to_use[4].split(',')))
...
def assign_nearest_splice(binding_file,filter_file):
    ...
    for line in ifile:
        ...
        peak_center = old_div((int(peak_start)+int(peak_end)),2)
